---
title: ME524 - Trabalho II
date: 11/25/2025
date-format: "DD/MM/YYYY"
author: Enzo Putton Tortelli (204256)
lang: pt
format: 
    typst:
      papersize: a4
...
```{r}
#| label: fig-density-tmulti
#| fig-cap: Densidades da t-Student multivariada simulada e dada pela função dmvt
#| echo: false
ggplot() +
...
## d)
```{r}
n <- 5000; mu <- c(-1, 2); df <- 40; sigma <- matrix(c(3, -1.3, -1.3, 1), ncol = 2)
normal <- MASS::mvrnorm(n = n, mu = mu, Sigma = sigma)
quiquadr <- rchisq(n = n, df = df)
t_student <- normal/sqrt(quiquadr/df) + mu
```
...
ggplot() +
  ...
  geom_contour(aes(x = x, y = y, z = z, color = 'Densidade'), data = xy)
```
Pelas @fig-density-tmulti e @fig-density-tmulti-qd, eu sei que a simulação está errada, mas não consegui arrumar. Dito isso, sei que com o aumento dos graus de liberdade, a t de Student multivariada converge para a normal multivariada.
...
```
Pela @tbl-q2-resultados, notamos que todos os métodos convergiram para os mesmos valores (duas casas decimais). O método NR foi o que convergiu com menos iterações, enquanto o NM, o que convergiu com mais. Além disso, o BFGS levou quase o dobro do número de iterações do L-BFGS-B. Dado o que aprendemos sobre a performce de cada um desses métodos, todos esses resultados são esperados. As funções utilizadas não deram *warning* em nenhum momento.
...
Caracterização estimativa pontual de $\beta_0$ (*Intercept*) geradas por bootstrap paramétrico. A linha pontilhada é estimativa pontual de $\beta_0$ para os dados originais.
:::
...
```
Caracterização da estimativa pontual de $\beta_1$ (*tempo*) geradas por bootstrap paramétrico. A linha pontilhada é a estimativa pontual de $\beta_1$ para os dados originais.
:::
...
#| echo: false
#| label: tbl-um
#| tbl-cap: Estatísticas sumárias das estimativas geradas.
summary(fit)[['summary']][1:2, ] %>% kable()
```
Pelas @fig-post-line, @fig-post-hist e @fig-post-densidade, podemos ver que as três cadeias convergem. Pela @tbl-um, notamos que as estimativas pontuais dadas pelas médias são muito próximas das estimativas do modelo glm frequentista.

{{< pagebreak >}}

# Questão V
Seja $F_n(t) = \frac{1}{n} \sum_{i = 1}^n \mathbb{1}_{(-\infty, t]}(X_i)$ e $X_i \sim F$.

Assim,

$$E[F_n(t)] =  \frac{1}{n} \sum_{i = 1}^n E[\mathbb{1}_{(-\infty, t]}(X_i)] \allowbreak
= \frac{1}{n} \sum_{i = 1}^n P(X_i \leq t) \allowbreak
= \frac{1}{n} \sum_{i = 1}^n F(t) \allowbreak
= F(t)$$

$$Var(F_n(t)) = Var\left(\frac{1}{n} \sum_{i = 1}^n \mathbb{1}_{(-\infty, t]}(X_i)\right) \allowbreak
= \frac{1}{n^2} \sum_{i = 1}^n Var\left(\mathbb{1}_{(-\infty, t]}(X_i)\right) \allowbreak
= \frac{1}{n^2} \sum_{i = 1}^n F(t) (1- F(t)) \allowbreak
= \frac{1}{n} F(t) (1- F(t))$$

Assim, pelo TCL:
$$\sqrt(n) \frac{F_n(t) - F(t)}{ F(t) (1- F(t)} \xrightarrow{D} N(0, 1)$$
